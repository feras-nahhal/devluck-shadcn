...
function ResultsPageInner() {
  ...
  const { getReport, getSession } = useCompanyAssessmentHandler()
  ...
  const fetchReport = useCallback(async () => {
    ...
    try {
      ...
      setError(err.response?.data?.message || err.response?.data?.detail || err.message || 'Failed to load report')
      setLoading(false)
    }
  }, [sessionId, getReport, getSession])
  ...
  const downloadPDF = async () => {
    ...
    try {
      const res = await api.get(
        `/api/company/assessment/sessions/${sessionId}/report/pdf`,
        {
          responseType: 'blob'
        }
      )
