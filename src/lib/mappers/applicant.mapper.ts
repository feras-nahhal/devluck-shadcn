...
// ✅ UPDATED SIGNATURE: Add optional ranks parameter
export function mapApplicant(
  app: any, 
  index: number, 
  ranks?: Record<string, number>  // ← NEW PARAMETER
): Applicant {
  ...
  return {
    ...
    // ✅ FIXED: Use ranks parameter (global ranking) OR fallback to student profileRanking
    profileRanking: ranks?.[applicantId] ?? student?.profileRanking ?? 0,
